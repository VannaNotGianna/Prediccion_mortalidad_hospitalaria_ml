...
\usepackage{graphicx}
\usepackage[caption=false,font=footnotesize]{subfig} % subfiguras compatibles con IEEEtran
\usepackage[section]{placeins} % evita que los floats se vayan a secciones/subsecciones posteriores
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{url}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/image9.png}
\caption{Proporción de valores faltantes por variable en el dataset SUPPORT.}
\label{fig:missingness}
\end{figure}
\FloatBarrier
\subsection{Tratamiento de valores faltantes e imputación}

Como parte del preprocesamiento, se realizó una revisión inicial de los valores faltantes presentes en el dataset. Este análisis permitió identificar las variables con observaciones incompletas y calcular el porcentaje de ausencia para cada una. Entre las variables con mayor proporción de valores faltantes se encontraron \texttt{adlp}, \texttt{urine}, \texttt{glucose}, \texttt{bun}, \texttt{totmcst}, \texttt{alb} e \texttt{income}.

Luego de realizar la limpieza relacionada con tipos de datos, valores imposibles y registros duplicados, se volvió a evaluar la proporción de valores faltantes. Se estableció como criterio eliminar aquellas columnas con más del 50\% de datos ausentes. Bajo este criterio, las variables \texttt{adlp} y \texttt{urine} fueron excluidas del conjunto de datos. La variable \texttt{glucose}, con aproximadamente 49.42\% de valores faltantes, se mantuvo debido a que no superó el umbral definido. Sus valores ausentes se conservaron para un posible tratamiento posterior o para evaluar su impacto durante la etapa de modelado.

Para las variables numéricas \texttt{bun}, \texttt{totmcst}, \texttt{alb}, \texttt{adls}, \texttt{bili}, \texttt{pafi}, \texttt{ph} y \texttt{edu}, los valores faltantes fueron imputados utilizando la mediana de cada variable. Esta medida fue seleccionada debido a su mayor robustez frente a la presencia de valores atípicos.

En el caso de las variables categóricas \texttt{income} y \texttt{sfdm2}, la imputación se realizó utilizando la moda, correspondiente al valor más frecuente de cada variable.

Finalmente, se realizó una verificación posterior al proceso de imputación para identificar los valores faltantes remanentes. Se mantuvieron sin imputar aquellos casos definidos por decisión de diseño, como \texttt{glucose}, así como otras variables que no fueron incluidas en esta etapa inicial de imputación, entre ellas \texttt{scoma}, \texttt{charges}, \texttt{totcst}, \texttt{avtisst}, \texttt{race}, \texttt{meanbp}, \texttt{wblc}, \texttt{hrt}, \texttt{resp}, \texttt{temp}, \texttt{crea} y \texttt{sod}. Estas variables serán revisadas nuevamente antes del entrenamiento de los modelos.
...
\begin{figure}[!t]
\centering
\subfloat[Distribución de \texttt{meanbp} según mortalidad hospitalaria.\label{fig:meanbp}]{%
  \includegraphics[width=\columnwidth]{figures/image3.png}%
}\\
\subfloat[Distribución de \texttt{hrt} según mortalidad hospitalaria.\label{fig:hrt}]{%
  \includegraphics[width=\columnwidth]{figures/image4.png}%
}\\
\subfloat[Distribución de \texttt{resp} según mortalidad hospitalaria.\label{fig:resp}]{%
  \includegraphics[width=\columnwidth]{figures/image8.png}%
}
\caption{Distribuciones de variables clínicas según mortalidad hospitalaria (1/2).}
\label{fig:dist_clinicas_a}
\end{figure}

\begin{figure}[!t]
\centering
\subfloat[Distribución de \texttt{crea} según mortalidad hospitalaria.\label{fig:crea}]{%
  \includegraphics[width=\columnwidth]{figures/image6.png}%
}\\
\subfloat[Distribución de \texttt{bili} según mortalidad hospitalaria.\label{fig:bili}]{%
  \includegraphics[width=\columnwidth]{figures/image5.png}%
}\\
\subfloat[Distribución de \texttt{alb} según mortalidad hospitalaria.\label{fig:alb}]{%
  \includegraphics[width=\columnwidth]{figures/image1.png}%
}
\caption{Distribuciones de variables clínicas según mortalidad hospitalaria (2/2).}
\label{fig:dist_clinicas_b}
\end{figure}
